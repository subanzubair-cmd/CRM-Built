...
export function EntitySendSmsModal({ entityType, entityId, contacts, label, onClose }: Props) {
  const initialContactId = contacts[0]?.id ?? ''
  ...
  // Fetch Twilio numbers — just use the first one as default
  useEffect(() => {
    let cancelled = false
  ...
  const sendLabel = wantSchedule && scheduleDate && scheduleTime ? 'Schedule Message' : 'Send Right Away'
        ...
        <div className="px-6 py-5 space-y-5 overflow-y-auto flex-1">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <label className="text-xs font-semibold text-gray-700">Sending From</label>
            ...
            {showFromPicker && twilioNumbers.length > 1 && (
              <div className="mt-2 border border-gray-200 rounded-lg p-3 bg-white space-y-3">
                ...
                <p className="text-xs text-gray-500">This number will be used for this message only.</p>
                <button
                  type="button"
                  onClick={() => setShowFromPicker(false)}
                  disabled={!selectedFromNumber}
                  className="mt-1 w-full flex items-center justify-center gap-1.5 bg-blue-600 hover:bg-blue-700 disabled:bg-blue-300 text-white text-sm font-medium py-2 rounded-lg transition-colors"
                >
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-xs text-gray-400">({charCount}/{segmentCount})</span>
            </div>
            <textarea
              value={message}
          ...
          <div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`px-4 py-1.5 text-xs font-medium rounded-lg border transition-colors ${wantSchedule ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-600 border-gray-200 hover:border-gray-300'}`}
              >
                Yes
              </button>
              <button
