        ...
        {selected === 'telnyx' && (
          <>
            ...
            <FieldRow
              ...
              mono
              hint="ID of your Voice API Application (Telnyx → Voice → Voice API & Apps → Applications). Required for inbound call routing — your numbers must be assigned to this application via connection_id."
            />
            <FieldRow
              label="Voice Connection ID (SIP — for browser calls)"
              value={fields.voiceConnectionId ?? ''}
              onChange={(v) => setField('voiceConnectionId', v)}
              mono
              hint="Optional. ID of a Telnyx SIP Connection (Credentials type) — required ONLY for browser-based outbound calls (WebRTC softphone). Telnyx → Voice → SIP Connections → Create New → Connection Type = Credentials → copy ID. Leave blank if you don't need browser dialing."
            />
            <FieldRow
              label="SIP Username (browser softphone login)"
              value={fields.sipUsername ?? ''}
              onChange={(v) => setField('sipUsername', v)}
              mono
              hint="Optional. Username generated by Telnyx when you save the Credentials-type SIP Connection above. Found in Mission Control → Voice → SIP Connections → your connection → Credentials tab. The browser softphone will register with this username + the password below instead of requesting a JWT — simpler setup, no /v2/telephony_credentials round-trip."
            />
            <FieldRow
              label="SIP Password"
              value={fields.sipPassword ?? ''}
              onChange={(v) => setField('sipPassword', v)}
              type="password"
              hint="Optional. Password from the same SIP Connection's Credentials tab. Stored encrypted at rest. Only required if SIP Username is set."
            />
            <FieldRow
              label="Public Key *"
