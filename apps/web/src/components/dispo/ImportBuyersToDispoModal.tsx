...
/**
 * ImportBuyersToDispoModal — 3-step wizard
 *
 *  Step 1 – Upload CSV + preview first 5 rows
 *  Step 2 – Map CSV columns → system fields  +  choose dispo stage
 *  Step 3 – Import (synchronous) + results summary
 *
 * The import calls POST /api/properties/[id]/buyer-matches/import
 * with { dispoStage, rows } where rows are plain objects
...
// ─────────────────────────────────────────────
// System field definitions (dispo subset only)
// ─────────────────────────────────────────────
...
export function ImportBuyersToDispoModal({ propertyId, stages, onClose }: Props) {
  ...
  async function runImport() {
    ...
    setImportError('')
    try {
      // Build mapped rows from parsed CSV + columnMap
      const mappedRows = parsed.rows.map((rawRow) => {
        ...
        for (const [csvHeader, sysKey] of Object.entries(columnMap)) {
          ...
          if (sysKey === 'fullName') {
            ...
            if (!obj['firstName']) obj['firstName'] = parts[0] ?? ''
            if (!obj['lastName']) obj['lastName'] = parts.slice(1).join(' ')
          } else {
            if (val) obj[sysKey] = val
          }
        }
        return obj
      ...
      const res = await fetch(`/api/properties/${propertyId}/buyer-matches/import`, {
        ...
        body: JSON.stringify({ dispoStage: selectedStage, rows: mappedRows }),
      })
...
// ─────────────────────────────────────────────
// Step 2 – Column Mapping + Stage Picker
// ─────────────────────────────────────────────
