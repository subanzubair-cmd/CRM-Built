...
/**
 * Table showing soft-deleted (inactive) vendors with Restore and
...
interface Props {
  rows: InactiveVendorRow[]
  total: number
}

export function InactiveVendorsTable({ rows, total }: Props) {
  ...
  async function restore(id: string, name: string) {
    ...
    try {
      ...
      toast.success(`"${name}" restored to active vendors.`)
      router.refresh()
    } catch (e: any) {
  ...
  async function hardDelete(id: string, name: string) {
    setBusyId(id)
    try {
      const res = await fetch(`/api/vendors/${id}?permanent=1`, { method: 'DELETE' })
      if (!res.ok) throw new Error('Failed.')
  ...
  if (rows.length === 0) {
    return (
      <div className="bg-white border border-gray-200 rounded-xl flex flex-col items-center justify-center h-48">
        ...
        <p className="text-sm text-gray-400">No inactive vendors</p>
        <p className="text-xs text-gray-300 mt-1">
          Deleted or deactivated vendors will appear here.
        </p>
      </div>
    )
  }
    ...
    <>
      <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-2.5 mb-4 flex items-center gap-2">
        ...
        <p className="text-[13px] text-amber-800">
          These vendors were soft-deleted or deactivated. You can restore them back to active status or permanently delete them.
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="text-[11px] text-gray-400 px-4 py-2 border-b border-gray-100">
          {total} inactive vendor{total !== 1 ? 's' : ''}
        </div>
