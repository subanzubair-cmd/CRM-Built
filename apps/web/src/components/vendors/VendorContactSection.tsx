...
export function VendorContactSection({ vendorId, phones, emails }: Props) {
  ...
  return (
    <>
      ...
      {/* Modals */}
      <SendSmsModal open={!!smsTarget} onClose={() => setSmsTarget(null)} toPhone={smsTarget ?? ''} entityType="vendor" entityId={vendorId} />
      <LogCallModal open={!!callTarget} onClose={() => setCallTarget(null)} toPhone={callTarget ?? ''} entityType="vendor" entityId={vendorId} />
      <LogEmailModal open={!!emailTarget} onClose={() => setEmailTarget(null)} toEmail={emailTarget ?? ''} entityType="vendor" entityId={vendorId} />
    </>
  )
}
