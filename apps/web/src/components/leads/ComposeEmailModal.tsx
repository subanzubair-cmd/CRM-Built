...
export function ComposeEmailModal({
  ...
  const defaultContact =
    (defaultContactId
      ? emailContacts.find((c) => c.id === defaultContactId)
      : undefined) ?? emailContacts[0] ?? null
        ...
        <div className="px-6 py-5 space-y-5 overflow-y-auto flex-1">
          ...
          {emailContacts.length === 0 && (
            <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-lg px-4 py-3 text-sm">
              No contacts with email addresses found for this property. Please add an email address
              to a contact before sending.
            </div>
          )}
          ...
          <div>
            <button
              ...
              onClick={() => {
                /* placeholder — file picker integration */
              }}
            >
