...
export function AddBuyerModal({ open, onClose }: Props) {
  ...
  const [dupWarning, setDupWarning] = useState<{ message: string; existingBuyerId: string } | null>(null)
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (!res.ok) {
      if (res.status === 409 && json.existingBuyerId) {
        setDupWarning({ message: json.error, existingBuyerId: json.existingBuyerId })
        return
      }
