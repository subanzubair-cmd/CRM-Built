...
export default async function RentalDetailPage({ params }: PageProps) {
  ...
  const [commStatsMap, adjacentIds] = await Promise.all([
    ...
    getAdjacentPropertyIds(property.id, 'IN_RENTAL'),
  ])
    ...
    details: (
      <>
        <AnalyticsTimeline
          ...
          contactName={property.contacts?.[0]?.contact ? `${property.contacts[0].contact.firstName} ${property.contacts[0].contact.lastName ?? ''}`.trim() : undefined}
        ...
        {property.soldAt && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4">
            <h3 className="text-sm font-semibold text-blue-800 mb-2">Rental Information</h3>
            <dl className="space-y-1.5 text-sm">
              <div className="flex justify-between">
                <dt className="text-blue-600">Rental Date</dt>
