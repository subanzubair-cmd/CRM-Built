...
import { hasPermission, requirePermission } from '@/lib/auth-utils'
import { z } from 'zod'
 ...
 * POST  admin only — writes the company timezone for everyone.
 *
 * Body shape: { timezone: string }   (IANA zone, e.g. 'America/Chicago')
 */
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ ok: true, data: { timezone: parsed.data.timezone } })
}
