...
export async function GET(req: NextRequest) {
  ...
  const [rows, total] = await Promise.all([
    ...
    BulkSmsBlast.count({ where: { module: 'BUYERS' as any } }),
  ])
...
export async function POST(req: NextRequest) {
  ...
  if (!buyerIds?.length && !filter) {
    return NextResponse.json(
      { error: 'Provide buyerIds or filter to choose recipients' },
      { status: 422 },
    )
  }

  // Resolve recipients into a deduped phone list. resolveBuyerRecipients
  // returns one entry per (subjectType, subjectId) pair with a valid
  // phone — opted-out / phoneless contacts are excluded here so the
  // recipient_count counter on the blast row is honest.
  const recipients = await resolveBuyerRecipients({
    buyerIds: buyerIds ?? [],
    filter: (filter as BuyerFilterParams | undefined) ?? null,
  })

  if (recipients.length === 0) {
    return NextResponse.json(
      { error: 'No reachable buyers matched. Check filter / phone numbers.' },
      { status: 422 },
    )
  }
